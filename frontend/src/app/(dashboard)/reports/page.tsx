...
export default function ReportsPage() {
  ...
  async function handleGenerate() {
    ...
    if (res.success && res.data) {
      ...
      // Download as PDF
      const reportData = res.data.report_data as unknown as ReportData | undefined;
      const data = reportData || preview;
      if (data) {
        generatePdfFromData(data);
      } else {
        // Fallback: download JSON if no structured data available
  ...
  function generatePdfFromData(data: ReportData) {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>${fw} Report</title><style>body{font-family:system-ui,sans-serif;padding:40px;max-width:800px;margin:0 auto}h1{color:#1E3A5F}h2{color:#1E3A5F;margin-top:24px}table{width:100%;border-collapse:collapse;margin:16px 0}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#f3f4f6}.stat{display:inline-block;margin:8px 16px 8px 0;padding:12px;background:#f3f4f6;border-radius:8px;min-width:120px;text-align:center}.stat-label{font-size:12px;color:#666}.stat-value{font-size:18px;font-weight:bold;color:#1E3A5F}</style></head><body>`);
    printWindow.document.write(`<h1>AquaVera — ${fw}</h1>`);
    printWindow.document.write(`<p>${data.companyName} · ${new Date(data.periodStart).toLocaleDateString()} — ${new Date(data.periodEnd).toLocaleDateString()}</p>`);
    printWindow.document.write(`<div class="stat"><div class="stat-label">Water Usage</div><div class="stat-value">${data.totalWaterUsage.toLocaleString()} L</div></div>`);
    printWindow.document.write(`<div class="stat"><div class="stat-label">WSC Retired</div><div class="stat-value">${data.totalWscRetired.toLocaleString()}</div></div>`);
    printWindow.document.write(`<div class="stat"><div class="stat-label">Liters Offset</div><div class="stat-value">${data.equivalentLitersOffset.toLocaleString()} L</div></div>`);
    printWindow.document.write(`<div class="stat"><div class="stat-label">Net Offset</div><div class="stat-value">${data.netOffsetPercentage}%</div></div>`);
    if (data.projectBreakdown && data.projectBreakdown.length > 0) {
      printWindow.document.write(`<h2>Source Projects</h2><table><tr><th>Project</th><th>Watershed</th><th>Type</th><th>WSC</th></tr>`);
      data.projectBreakdown.forEach((p) => { printWindow.document.write(`<tr><td>${p.projectName}</td><td>${p.watershed}</td><td>${p.creditType}</td><td>${p.quantity}</td></tr>`); });
    ...
    if (data.retirements && data.retirements.length > 0) {
      ...
      data.retirements.forEach((r) => { printWindow.document.write(`<tr><td>${r.quantity}</td><td>${r.liters.toLocaleString()}</td><td>${new Date(r.date).toLocaleDateString()}</td><td>#${r.nftSerial}</td><td>${r.burnTxId}</td></tr>`); });
      printWindow.document.write(`</table>`);
    }
